           ...
           stringsAsFactors = default.stringsAsFactors())
  {
    ...
    for (i in seq_len(n)) {
      ...
      if (mirn && nrows[i] > 0L) {
        ...
        if (length(nc) && any(nc))
          row.names <-
          .data.row.names(row.names, rowsi, i, check.rows, rows.names)
      }
      nrows[i] <- abs(nrows[i])
      vlist[[i]] <- xi
    }
        ...
        for (j in seq_along(xi)) {
          ...
          if (is.vector(xi1) || is.factor(xi1))
            ...
            xi[[j]] <- structure(rep(xi1, length.out = nr),
                                 class = class(xi1))
          else if (inherits(xi1, "Date") ||
                   inherits(xi1, "POSIXct"))
            xi[[j]] <- rep(xi1, length.out = nr)
          else {
            fixed <- FALSE
            break
    ...
    if (any(noname))
      vnames[noname] <-
      paste("Var", seq_along(vnames), sep = ".")[noname]
    if (check.names)
      vnames <- make.names(vnames, unique = TRUE)
    names(value) <- vnames
    if (!mrn) {
      # non-null row.names arg was supplied
...
.data.row.names <-
  function(current, new, i, check.rows, row.names) {
    if (check.rows && is.null(row.names)) {
      ...
      if (all(current == new) || all(current == ""))
        return(new)
      stop(gettextf(
        "mismatch of row names in arguments of 'data.frame\', item %d",
        i
      ),
      domain = NA)
    } else {
