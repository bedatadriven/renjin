...
public class DollarAssignFunction extends SpecialFunction {
  ...
  public static SEXP assign(Context context, Environment rho, FunctionCall call, SEXP object, String name, SEXP value) {
    if(object.isObject()) {
      ...
      SEXP[] evaluatedArguments = new SEXP[3];
      evaluatedArguments[0] = object;
      evaluatedArguments[1] = StringVector.valueOf(name);
      evaluatedArguments[2] = value;

      SEXP genericResult = S3.tryDispatchFromPrimitive(context, rho, call, "$<-", null, argumentNames, evaluatedArguments);
