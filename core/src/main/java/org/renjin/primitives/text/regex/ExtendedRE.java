...
public class ExtendedRE implements Serializable, RE {
  ...
  // Limits
  static final int MAX_NODE = 65536;            // Maximum number of nodes in a program
  static final int MAX_PAREN = 16;              // Number of paren pairs (only 9 can be backrefs)

  // Node layout constants
  static final int OFFSET_OPCODE = 0;            // Opcode offset (first character)
  static final int OFFSET_OPDATA = 1;            // Opdata offset (second char)
  static final int OFFSET_NEXT = 2;            // Next index offset (third char)
  static final int NODE_SIZE = 3;            // Node size (in chars)
  ...
  protected int matchNodes(int firstNode, int lastNode, int idxStart) {
    ...
    for (int node = firstNode; node < lastNode; ) {
      ...
      next = node + (short) instruction[node + OFFSET_NEXT];
      opdata = instruction[node + OFFSET_OPDATA];
      ...
      switch (opcode) {
        ...
        case OP_STAR: {
          ...
          if ((idxNew = matchNodes(node + NODE_SIZE, MAX_NODE, idx)) != -1) {
            return idxNew;
          }
        ...
        case OP_PLUS: {
          ...
          if ((idxNew = matchNodes(next, MAX_NODE, idx)) != -1) {
            return idxNew;
          }
          ...
          node = next + (short) instruction[next + OFFSET_NEXT];
          continue;
        }
        ...
        case OP_RELUCTANTSTAR: {
          ...
          return matchNodes(node + NODE_SIZE, next, idx);
        }

        case OP_RELUCTANTPLUS: {
          // Continue matching the rest without using the reluctant subexpr
          if ((idxNew = matchNodes(next + (short) instruction[next + OFFSET_NEXT], MAX_NODE, idx)) != -1) {
            return idxNew;
          }
        ...
        case OP_BRANCH: {
          ...
          do {
            ...
            nextBranch = (short) instruction[node + OFFSET_NEXT];
            node += nextBranch;
          }
  ...
  protected boolean matchAt(int i) {
    ...
    int idx;
    if ((idx = matchNodes(0, MAX_NODE, i)) != -1) {
      setParenEnd(0, idx);
      return true;
    }
