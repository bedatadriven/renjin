 ...
 * regular expression program interpretable by the RE evaluator class.
 *
...
public class RECompiler {
  ...
  // Node flags
  private static final int NODE_NORMAL = 0;                 // No flags (nothing special)
  private static final int NODE_NULLABLE = 1;                 // True if node is potentially null
  private static final int NODE_TOPLEVEL = 2;                 // True if top level expr

  // Special types of 'escapes'
  private static final int ESC_MASK = 0xffff0;           // Escape complexity mask
  private static final int ESC_BACKREF = 0xfffff;           // Escape is really a backreference
  private static final int ESC_COMPLEX = 0xffffe;           // Escape isn't really a true character
  private static final int ESC_CLASS = 0xffffd;           // Escape represents a whole class of characters
  ...
  static final HashMap<String, Character> hashPOSIX = new HashMap<>();

  static {
    hashPOSIX.put("alnum", ExtendedRE.POSIX_CLASS_ALNUM);
    hashPOSIX.put("alpha", ExtendedRE.POSIX_CLASS_ALPHA);
    hashPOSIX.put("blank", ExtendedRE.POSIX_CLASS_BLANK);
    hashPOSIX.put("cntrl", ExtendedRE.POSIX_CLASS_CNTRL);
    hashPOSIX.put("digit", ExtendedRE.POSIX_CLASS_DIGIT);
    hashPOSIX.put("graph", ExtendedRE.POSIX_CLASS_GRAPH);
    hashPOSIX.put("lower", ExtendedRE.POSIX_CLASS_LOWER);
    hashPOSIX.put("print", ExtendedRE.POSIX_CLASS_PRINT);
    hashPOSIX.put("punct", ExtendedRE.POSIX_CLASS_PUNCT);
    hashPOSIX.put("space", ExtendedRE.POSIX_CLASS_SPACE);
    hashPOSIX.put("upper", ExtendedRE.POSIX_CLASS_UPPER);
    hashPOSIX.put("xdigit", ExtendedRE.POSIX_CLASS_XDIGIT);
    hashPOSIX.put("javastart", ExtendedRE.POSIX_CLASS_JSTART);
    hashPOSIX.put("javapart", ExtendedRE.POSIX_CLASS_JPART);
  }
  ...
  void nodeInsert(char opcode, int opdata, int insertAt) {
    ...
    ensure(ExtendedRE.NODE_SIZE);

    // Move everything from insertAt to the end down nodeSize elements
    System.arraycopy(instruction, insertAt, instruction, insertAt + ExtendedRE.NODE_SIZE, lenInstruction - insertAt);
    instruction[insertAt /* + RE.offsetOpcode */] = opcode;
    instruction[insertAt + ExtendedRE.OFFSET_OPDATA] = (char) opdata;
    instruction[insertAt + ExtendedRE.OFFSET_NEXT] = 0;
    lenInstruction += ExtendedRE.NODE_SIZE;
  }
  ...
  void setNextOfEnd(int node, int pointTo) throws RESyntaxException {
    ...
    int next = instruction[node + ExtendedRE.OFFSET_NEXT];
    ...
    while (next != 0 && node < lenInstruction) {
      ...
      node += next;
      next = instruction[node + ExtendedRE.OFFSET_NEXT];
    }
    ...
    if (node < lenInstruction) {
      ...
      instruction[node + ExtendedRE.OFFSET_NEXT] = (char) (short) offset;
    }
  }
  ...
  int node(char opcode, int opdata) {
    ...
    instruction[lenInstruction + ExtendedRE.OFFSET_OPDATA] = (char) opdata;
    instruction[lenInstruction + ExtendedRE.OFFSET_NEXT] = 0;
    lenInstruction += ExtendedRE.NODE_SIZE;

    // Return index of new node
    return lenInstruction - ExtendedRE.NODE_SIZE;
  }
  ...
  int characterClass() throws RESyntaxException {
    ...
    instruction[ret + ExtendedRE.OFFSET_OPDATA] = (char) range.num;
  ...
  int atom() throws RESyntaxException {
    ...
    instruction[ret + ExtendedRE.OFFSET_OPDATA] = (char) lenAtom;
    return ret;
  }
      ...
      switch (closureType) {
        case '{': {
          ...
          if (bracketOpt == bracketUnbounded) {
            ...
            setNextOfEnd(pos + ExtendedRE.NODE_SIZE, pos);
            break;
            ...
            for (int c = 0; c < bracketOpt; c++) {
              ...
              setNextOfEnd(opt[c] + ExtendedRE.NODE_SIZE, opt[c + 1]);
            }
          } else {
        ...
        case '?': {
          ...
          setNextOfEnd(ret, n);
          setNextOfEnd(ret + ExtendedRE.NODE_SIZE, n);
          break;
        }

        case '*': {
          nodeInsert(ExtendedRE.OP_STAR, 0, ret);
          setNextOfEnd(ret + ExtendedRE.NODE_SIZE, ret);
          break;
        }
  ...
  int expr(int[] flags) throws RESyntaxException {
    ...
    while (nextNodeOffset != 0 && currentNode < lenInstruction) {
      ...
      if (instruction[currentNode /* + RE.offsetOpcode */] == ExtendedRE.OP_BRANCH) {
        setNextOfEnd(currentNode + ExtendedRE.NODE_SIZE, end);
      }
      nextNodeOffset = instruction[currentNode + ExtendedRE.OFFSET_NEXT];
