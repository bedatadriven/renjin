...
public class REProgram implements Serializable {
  ...
  public void setInstructions(char[] instruction, int lenInstruction) {
    ...
    if (instruction != null && lenInstruction > 0) {
      final char firstOp = instruction[ExtendedRE.OFFSET_OPCODE];

      // If the first node is an atom
      if (firstOp == ExtendedRE.OP_ATOM) {
        // then get that atom as an prefix because there's no other choice
        int lenAtom = instruction[ExtendedRE.OFFSET_OPDATA];
        this.prefix = new char[lenAtom];
        System.arraycopy(instruction, ExtendedRE.NODE_SIZE, prefix, 0, lenAtom);
      ...
      for (int i = 0; i < lenInstruction; i += ExtendedRE.NODE_SIZE) {
        switch (instruction[i + ExtendedRE.OFFSET_OPCODE]) {
          case ExtendedRE.OP_ANYOF:
            i += (instruction[i + ExtendedRE.OFFSET_OPDATA] * 2);
            break;

          case ExtendedRE.OP_ATOM:
            i += instruction[i + ExtendedRE.OFFSET_OPDATA];
            break;
