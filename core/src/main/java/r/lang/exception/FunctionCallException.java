...
public class FunctionCallException extends RuntimeException {
  ...
  private static String formatMessage(FunctionCall call, PairList arguments, Exception e) {
    ...
    SEXP sexp = call.getAttribute(Symbols.SRC_REF);
    if(sexp instanceof IntVector) {
      SEXP srcfile = sexp.getAttribute(Symbols.SRC_FILE);
      message.append(" (").append(srcfile).append(": ").append(((IntVector) sexp).getElementAsInt(0)).append(")");
    }
