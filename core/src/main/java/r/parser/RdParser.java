...
import static r.util.CDefines.CAR;
import static r.util.CDefines.CDR;
import static r.util.CDefines.CHAR;
import static r.util.CDefines.CONS;
import static r.util.CDefines.PROTECT;
import static r.util.CDefines.R_NilValue;
import static r.util.CDefines.SETCAR;
import static r.util.CDefines.SETCDR;
import static r.util.CDefines.STRING_ELT;
import static r.util.CDefines.UNPROTECT;
import static r.util.CDefines.UNPROTECT_PTR;
import static r.util.CDefines._;
import static r.util.CDefines.error;
import static r.util.CDefines.getAttrib;
import static r.util.CDefines.install;
import static r.util.CDefines.isNull;
import static r.util.CDefines.length;
  ...
  yyr2_[] =
{
  ...
  private void setDynamicFlag(ListVector.Builder item, int flag) {
    if (flag != 0) {
      CDefines.setAttrib(item, install("dynamicFlag"), new IntVector(flag));
    }
  }

  private SEXP setDynamicFlag(SEXP item, int flag) {
    if (flag != 0) {
      return item.setAttribute(install("dynamicFlag"), new IntVector(flag));
    } else {
      return item;
    }
  }
