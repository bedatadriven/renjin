...
public class PatternTest {
  ...
  public void posixClasses() throws RESyntaxException {
    ...
    assertTrue( new ExtendedPattern("^[[:digit:]]+$").match("1249234") );
  }
  ...
  public void dashInCharacterClass() throws RESyntaxException {
    ...
    assertTrue( new ExtendedPattern("^[a-]+$").match("a-a--aa---a-aaa") );
    assertTrue( new ExtendedPattern("^[-a]+$").match("a-a--aa---a-aaa") );
    assertTrue( new ExtendedPattern("^[-3]+$").match("3-3-333---3") );
    assertFalse(new ExtendedPattern("^[-3]+$").match("23"));

    // make sure that normal character ranges still work
    assertTrue(new ExtendedPattern("^[a-z4]+$").match("qf444ee"));
  }
}
