		...
		contrasts.arg = NULL, xlev = NULL, ...)
{
	...
	# determine contrast functions for factors in the data frame
	factors <- sapply(data, function(x) is.factor(x) || is.logical(x))
	factors[attr(t, "response")] <- FALSE
	...
	defined.contrasts <- lapply(names(data), function(name) {
		...
		if(is.factor(x) || is.logical(x)) {
			if(!is.null(contrasts.arg[[name]])) {
				...
				as.vector(getOption("contrasts")["ordered"])
			} else {
				as.vector(getOption("contrasts")["unordered"])
			}
		}
	})
	...
	mm <- .Internal(model.matrix(t, data))
	
	which.factors <- !is.null(defined.contrasts)
	if(any(which.factors)) {
		contrasts <- defined.contrasts[which.factors]
		names(contrasts) <- names(data)[which.factors]
