...
read.table <- function(file, header = FALSE, sep = "", quote = "\"'", dec = ".",
     ...
     tableOptions$autoDetectHeader <- missing(header)
     tableOptions$header <- header
     tableOptions$fieldSep <- sep
     tableOptions$quote <- quote
     tableOptions$dec <- dec
     tableOptions$naStrings <- na.strings
   #  tableOptions$colNames <- col.names
     tableOptions$colClasses <- colClasses
     tableOptions$rowLimit <- nrows
     tableOptions$skip <- skip
     tableOptions$checkNames <- check.names
     tableOptions$fill <- fill
     tableOptions$stripWhitespace <- strip.white
     tableOptions$blankLinesSkip <- blank.lines.skip
     tableOptions$commentChar <- comment.char
     tableOptions$allowEscapes <- allowEscapes
     tableOptions$flush <- flush
     tableOptions$stringsAsFactors <- stringsAsFactors
     tableOptions$fileEncoding <- fileEncoding
     
     tableReader <-TableReader$new(tableOptions)
     tableReader$read(file)
...
#    do <- keep & !known # & !as.is
#    if(rlabp) {
#        do[1L] <- FALSE # don't convert "row.names"
#    }
#    for (i in (1L:cols)[do]) {
#        data[[i]] <-
...
#    }
#
#    as.data.frame(data, row.names = row.names)
}
